                    ...
                    {search.length > 0 || filteredMembers > 0 ? (
                        filteredMembers.map((filteredMember, index) => (
                          <tr key={filteredMember._id} 
                            className={`all-members-list personal-members-list ${!changeColor ? "all-members-list-black" : ""} ${isRename === member._id ? 'update-table' : isDelete === member._id ? 'delete-table' : ''} ${index % 2 === 0 ? "even-row" : "odd-row"}`}
                            >
                                  <td>
